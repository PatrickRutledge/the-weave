...
exports.GitPatternDetector = void 0;
const simple_git_1 = __importDefault(require("simple-git"));
class GitPatternDetector {
    ...
    detectBuildFailures(commits) {
        const buildKeywords = ['build failed', 'fix build', 'build error', 'build issue'];
        const matches = commits.filter(c => buildKeywords.some(kw => c.message.toLowerCase().includes(kw)));
        return {
            type: 'build_failures',
    ...
    detectCircularCommits(commits) {
        const reverts = commits.filter(c => c.message.toLowerCase().startsWith('revert'));
        const undos = commits.filter(c => c.message.toLowerCase().includes('undo') ||
    ...
    detectCommitClusters(commits) {
        ...
        return {
            ...
            examples: clusters.slice(0, 2).map(cluster => `${cluster.length} commits: ${cluster[0].message}`),
            severity: clusters.length > 3 ? 'high' : clusters.length > 1 ? 'medium' : 'low',
        };
    }
    detectRevertPattern(commits) {
        const reverts = commits.filter(c => c.message.toLowerCase().startsWith('revert'));
        return {
            type: 'reverts',
    ...
    generateRecommendations(patterns) {
        ...
        if (circular && circular.severity !== 'low') {
            recommendations.push('High number of reverts suggests unclear requirements or testing gaps');
        }
        const clusters = patterns.find(p => p.type === 'commit_clusters');
        if (clusters && clusters.severity === 'high') {
            recommendations.push('Commit clusters suggest areas of struggle - consider retrospective analysis');
        }
        const fixes = patterns.find(p => p.type === 'fix_pattern');
        if (fixes && fixes.severity === 'high') {
            recommendations.push('High ratio of fix commits - consider improving testing strategy');
        }
...
exports.GitPatternDetector = GitPatternDetector;
//# sourceMappingURL=git-pattern-detector.js.map